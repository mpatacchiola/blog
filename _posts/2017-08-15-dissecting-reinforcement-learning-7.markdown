...
If you look to the equation what I added is the new term $$x_{1} x_{2} w_{3}$$. This term introduces a relationship between the first two features. Now the surface represented by the equation is no more a plane but an [hyperbolic paraboloid](https://en.wikipedia.org/wiki/Paraboloid), a saddle-like surface which perfectly adapt to the XOR-world. In the repository you will find another script called `xor_paraboloid.py` containing an implementation of this new function. Running the script with the same parameters used for the linear case we end up with the following plot:
...
Here the paraboloid is represented from four different point of view. 
