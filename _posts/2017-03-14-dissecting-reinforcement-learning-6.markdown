...
How can we deal with this problem using a discrete approach? What we can do is dividing the continuous state-action space in chunks. The **discretization** allows using a lookup table to store all we need.
I call **bins** the discrete containers where both position and velocity are stored. In Numpy is easy to create this containers using the `numpy.linspace()` function. For example, in our problem the **position** is a continuous value in the range $$[-1.2, 0.5]$$ which can be divided in 10 chunks. When the car is at -1.2 it is in the first bin. 
In the script I defined the policy matrix as a square matrix having size `tot_bins`. However it is possible to differently discretize velocity and position, obtaining a rectangular matrix.
...
Here I will use the temporal differencing method called SARSA which I introduced in the [third Post](https://mpatacchiola.github.io/blog/2017/01/29/dissecting-reinforcement-learning-3.html). However I suggest you to use other methods to check the different performances you may obtain. Only a few changes are required in order to run the code of the previous posts. In this example I trained the policy for $$10^{5}$$ episodes ($$\gamma=0.999$$), using an epsilon decayed value which helped exploration in the first part of the training. The script automatically saves a gif and plots every $$10^{4}$$ episodes. The following is the graph of the **cumulated reward per episode**, where the light line is the raw data and the dark line a moving average of 500 episodes:
...
One of the policy that the algorithm otained is very efficient and allows to reach the target position in less than 7 seconds. However is not the best one, as it is possible to see from the step plot (light-blue curve) there are policies that can reach the goal in around 40 steps (4 seconds).
...
The discrete approach worked well in the mountain car problem. However there are some possible issues that may occur. 

Inverted Pendulum
------------------
The inverted pendulum is another classical problem, which is considered a benchmark in control theory. The problem consists of a pole hinged on a cart which must be moved in order to keep the pole in vertical position. The inverted pendulum is well described in chapter 4.5.1 of [Sugiyama's book](https://www.crcpress.com/Statistical-Reinforcement-Learning-Modern-Machine-Learning-Approaches/Sugiyama/p/book/9781439856895). Here I will use the same mathematical notation.
...
$$ \dot{\phi_{t+1}} = \dot{\phi_{t}} + \frac{g \ sin(\phi_t) - \alpha \ m \ d \ (\dot{\phi}_t)^2 \  sin(2\phi_t)/2 + \alpha \ cos(\phi_t) \ a_t }{ 4l/3 - \alpha \ w \ d \ cos^2(\phi_t)} \Delta t$$

Here $$ \alpha = 1 / M+m $$, and $$ a_t $$ is the action at time $$ t $$.
The **reward** is updated considering the cosine of the angle $$ \phi $$, meaning larger the angle lower the reward. The reward is 0.0 when the pole is horizontal and 1.0 when vertical.
...
In the cleaning robot example the robot moved in a flat 2D environment. Now it is time to add another dimension. We have to train an autonomous **drone** to **land on a marker**. The drone moves in a discrete 3D world, representing a cubic room. The marker is always in the same point (the centre of the room). The rules are similar to the gridworld, if the drone hit one of the wall it bounce back to the previous position. Landing on the marker leads to a positive reward of +1.0, while landing on another point leads to a negative reward of -1.0.
