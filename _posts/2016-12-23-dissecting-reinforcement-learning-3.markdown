...
SARSA uses GPI to improve the policy $$ \pi $$. The $$ \text{Target} $$ is estimated through $$ Q(s_{t+1}, a_{t+1}) $$ which is based on the action $$ a_{t+1} $$ sampled from the policy $$ \pi $$. In SARSA improving $$ \pi $$ means improving the estimation returned by $$ Q(s_{t+1}, a_{t+1}) $$. In Q-learning we have two policies $$ \pi $$ and $$ \mu $$. The action value $$ a $$ necessary to estimate $$ Q(s_{t},a) $$ is directly taken from the Q-function using the $$ \text{ max } $$ operator. Q-learning makes an update based on the greedy Q-value of the successor state, $$ s_{t+1} $$, while SARSA uses the Q-value of the action $$ a_{t+1} $$ actually chosen by the learning policy. This makes SARSA an on-policy algorithm, and therefore its conditions for convergence depend a great deal on the learning policy. Q-learning does not depend on the policy and convergence can be achieved also following a completely random policy. Now I would like to detail how the $$ \text{Target} $$ term is build in Q-learning. First of all, let's remember that the value of $$ a_{t+1} $$ cannot be sampled from the policy $$ \mu $$ because $$ \mu $$ is not updated during the training and using it would **break the GPI scheme**. Having the Q-function available we can update at each time step the policy $$ \pi $$ through $$ \underset{a}{\text{ max }} Q(s_{t+1}, a) $$ and getting $$ \pi(s_{t+1}) $$ as follows:
